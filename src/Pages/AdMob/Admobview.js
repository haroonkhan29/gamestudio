...
const Admobview = () => {
  ...
  const handleView = (data) => {
    ...
    const images = admobData.map((item) => ({
      id: item._id,
      imageSrc: `http://3.140.190.237:3002/uploads/${item.profilePic}`,
      date: item.date,
  ...
  const handleDelete = async (index, id) => {
    try {
      await axios.delete(`http://3.140.190.237:3002/admob/${id}`);
  ...
  const fetchData = async (formattedDate) => {
    try {
      const apiUrl = "http://3.140.190.237:3002/admob";
          ...
          {selectedData && (
            <div className="selected">
              ...
              {selectedData.profilePic && (
                <img
                  src={`http://3.140.190.237:3002/uploads/${selectedData.profilePic}`}
                  alt="Selected"
                        ...
                        <td>
                          {data.profilePic && (
                            <img
                              ...
                              onClick={() =>
                                openImageModal(
                                  `http://3.140.190.237:3002/uploads/${data.profilePic}`
                                )
                              }
                            />
                          )}
